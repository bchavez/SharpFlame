...
namespace SharpFlame.Old
{
    public partial class frmGenerator
    {
        ...
        private int ValidateTextbox(TextBox TextBoxToValidate, double Min, double Max, double Multiplier)
        {
            ...
            Result = Math.Floor(MathUtil.ClampDbl(dblTemp, Min, Max) * Multiplier).ToInt();
        ...
        private Result FinishTextures()
        {
            ...
            else
            {
                ...
                Alignments.Normal = new XYInt(Math.Floor(Generator.Map.Terrain.TileSize.X / 2.0D).ToInt(), Math.Floor(Generator.Map.Terrain.TileSize.Y / 2.0D).ToInt());
