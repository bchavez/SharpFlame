...
namespace SharpFlame
{
	public class clsObjectData
	{
		...
		public class clsTextFile
		{
			...
			{
				clsResult Result = new clsResult("Loading comma separated file " + System.Convert.ToString(ControlChars.Quote) + SubDirectory + System.Convert.ToString(ControlChars.Quote));
			...
			{
				clsResult Result = new clsResult("Loading names file " + System.Convert.ToString(ControlChars.Quote) + SubDirectory + System.Convert.ToString(ControlChars.Quote));
...
MonoContinueDo:
		...
		{
			clsResult ReturnResult = new clsResult("Loading object data from " + System.Convert.ToString(ControlChars.Quote) + Path + System.Convert.ToString(ControlChars.Quote));
			...
			SubDirNames = "messages" + System.Convert.ToString(modProgram.PlatformPathSeparator) + "strings" + System.Convert.ToString(modProgram.PlatformPathSeparator) + "names.txt";
			...
			foreach (clsTextFile tempLoopVar_TextFile in CommaFiles)
			{
				...
				if (!Result.HasProblems)
				{
					if (TextFile.CalcIsFieldCountValid())
					{
						...
						{
							ReturnResult.ProblemAdd("An entry in field " + System.Convert.ToString(TextFile.UniqueField) + " was not unique for file " + TextFile.SubDirectory + ".");
						}
					}
					else
					{
			...
			foreach (string tempLoopVar_Text in TexFiles)
			{
				...
				{
					clsResult Result = new clsResult("Loading texture page " + System.Convert.ToString(ControlChars.Quote) + Text + System.Convert.ToString(ControlChars.Quote));
						...
						else if (InCommentBlock)
						{
							if (Lines[LineNum][CharNum] == '*')
							{
								...
								else if (Lines[LineNum][CharNum] == '/')
								{
									...
									InCommentBlock = false;
									Lines[LineNum] = Strings.Left(Lines[LineNum], CommentStart) + Strings.Right(Lines[LineNum], System.Convert.ToInt32(Lines[LineNum].Length - (CommentStart + CommentLength)));
