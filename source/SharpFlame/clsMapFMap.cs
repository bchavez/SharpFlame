...
namespace SharpFlame
{
	public partial class clsMap
	{
		...
		{
			clsResult ReturnResult = new clsResult("Writing FMap to " + System.Convert.ToString(ControlChars.Quote) + Path + System.Convert.ToString(ControlChars.Quote));
...
ReturnResult.WarningAdd("Error: " + System.Convert.ToString(WarningCount) + " units were of an unhandled type.");
		...
		{
			clsResult ReturnResult = new clsResult("Loading FMap from " + System.Convert.ToString(ControlChars.Quote) + Path + System.Convert.ToString(ControlChars.Quote));
				...
				for (Y = 0; Y <= Terrain.TileSize.Y; Y++)
				{
					for (X = 0; X <= Terrain.TileSize.X; X++)
					{
						...
						else if (Value >= Painter.TerrainCount)
						{
							if (WarningCount < 16)
							{
								ReturnResult.WarningAdd("Painted terrain at vertex " + System.Convert.ToString(X) + ", " + System.Convert.ToString(Y) + " was invalid.");
							}
							WarningCount++;
							Terrain.Vertices[X, Y].Terrain = null;
						}
					...
					for (X = 0; X <= Terrain.TileSize.X - 1; X++)
					{
						...
						switch (PartValue)
						{
							...
							default:
								...
								{
									ReturnResult.WarningAdd("Down side value for tile " + System.Convert.ToString(X) + ", " + System.Convert.ToString(Y) + " was invalid.");
								}
				...
				else
				{
					...
					if (INIObjects.Objects[A].Type != clsUnitType.enumType.Unspecified)
					{
						...
						if (IsDesign)
						{
							...
							{
								DroidDesign.Turret1 = modProgram.ObjectData.FindOrCreateTurret(INIObjects.Objects[A].TurretTypes[0], System.Convert.ToString(INIObjects.Objects[A].TurretCodes[0]));
							...
							{
								DroidDesign.Turret2 = modProgram.ObjectData.FindOrCreateTurret(INIObjects.Objects[A].TurretTypes[1], System.Convert.ToString(INIObjects.Objects[A].TurretCodes[1]));
							...
							{
								DroidDesign.Turret3 = modProgram.ObjectData.FindOrCreateTurret(INIObjects.Objects[A].TurretTypes[2], System.Convert.ToString(INIObjects.Objects[A].TurretCodes[2]));
						...
						else
						{
							...
							if (UnitType.IsUnknown)
							{
								...
								{
									ReturnResult.WarningAdd(ControlChars.Quote + INIObjects.Objects[A].Code + System.Convert.ToString(ControlChars.Quote) + " is not a loaded object.");
								}
