...
namespace SharpFlame
{
	...
	public abstract class clsOptionInterface
	{
		...
		abstract public modLists.ConnectedListLink<clsOptionInterface, clsOptionGroup> GroupLink {get;}
		
		abstract public string SaveKey {get;}
		abstract public object DefaultValueObject {get;}
		abstract public bool IsValueValid(object value);
	}
...
public override string SaveKey
	...
	public class clsOptionProfile : clsINIRead.clsTranslator
	{
		...
		{
			abstract public object ValueObject {get;}
			abstract public clsChangeInterface GetCopy();
		}
...
public clsOptionGroup Options
		...
		public clsResult INIWrite(clsINIWrite file)
		{
			...
			foreach (clsOptionInterface item in _Options.Options)
			{
				...
				{
					returnResult.ProblemAdd("Value for option " + System.Convert.ToString(ControlChars.Quote) + item.SaveKey + System.Convert.ToString(ControlChars.Quote) + " could not be written because it is of type " + optionValue.GetType().FullName);
				}
				if (valueText != null)
				{
