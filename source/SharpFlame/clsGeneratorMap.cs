...
public int GetTotalPlayerCount
		...
		public clsResult GenerateLayout()
		{
			...
			modMath.sXY_int SymmetrySize = new modMath.sXY_int();
			
			SymmetrySize.X = (int) (TileSize.X * modProgram.TerrainGridSpacing / SymmetryBlockCountXY.X);
			SymmetrySize.Y = (int) (TileSize.Y * modProgram.TerrainGridSpacing / SymmetryBlockCountXY.Y);
			
			//create passage nodes
			
			int PassageRadius = (int) (128.0F * NodeScale);
			int MaxLikelyPassageNodeCount = 0;
			MaxLikelyPassageNodeCount = (int) (Math.Ceiling(System.Convert.ToDecimal(2.0D * TileSize.X * 128 * TileSize.Y * 128 / (Math.PI * PassageRadius * PassageRadius))));
			...
			{
				EdgeSections.X = System.Convert.ToInt32(Conversion.Int((TileSize.X * modProgram.TerrainGridSpacing - EdgeOffset * 2.0D) / (NodeScale * modProgram.TerrainGridSpacing * 2.0F)));
				EdgeSectionSize.X = (TileSize.X * modProgram.TerrainGridSpacing - EdgeOffset * 2.0D) / EdgeSections.X;
				EdgeSections.X--;
			}
			else
			{
				EdgeSections.X = (int) (Conversion.Int((TileSize.X * modProgram.TerrainGridSpacing / SymmetryBlockCountXY.X - EdgeOffset) / (NodeScale * modProgram.TerrainGridSpacing * 2.0F) - 0.5D));
				EdgeSectionSize.X = System.Convert.ToDouble((TileSize.X * modProgram.TerrainGridSpacing / SymmetryBlockCountXY.X - EdgeOffset) / (System.Convert.ToDouble(Conversion.Int((TileSize.X * modProgram.TerrainGridSpacing / SymmetryBlockCountXY.X - EdgeOffset) / (NodeScale * modProgram.TerrainGridSpacing * 2.0F) - 0.5D)) + 0.5D));
			}
			if (SymmetryBlockCountXY.Y == 1)
			{
				EdgeSections.Y = System.Convert.ToInt32(Conversion.Int((TileSize.Y * modProgram.TerrainGridSpacing - EdgeOffset * 2.0D) / (NodeScale * modProgram.TerrainGridSpacing * 2.0F)));
				EdgeSectionSize.Y = (TileSize.Y * modProgram.TerrainGridSpacing - EdgeOffset * 2.0D) / EdgeSections.Y;
				EdgeSections.Y--;
			}
			else
			{
				EdgeSections.Y = System.Convert.ToInt32(Conversion.Int((TileSize.Y * modProgram.TerrainGridSpacing / SymmetryBlockCountXY.Y - EdgeOffset) / (NodeScale * modProgram.TerrainGridSpacing * 2.0F) - 0.5D));
				EdgeSectionSize.Y = System.Convert.ToDouble((TileSize.Y * modProgram.TerrainGridSpacing / SymmetryBlockCountXY.Y - EdgeOffset) / (System.Convert.ToDouble(Conversion.Int((TileSize.Y * modProgram.TerrainGridSpacing / SymmetryBlockCountXY.Y - EdgeOffset) / (NodeScale * modProgram.TerrainGridSpacing * 2.0F) - 0.5D)) + 0.5D));
			}
			
			PassageNodeCount = 0;
			for (Y = 0; Y <= EdgeSections.Y; Y++)
			{
				if (!MakePassageNodes(new modMath.sXY_int(EdgeOffset, EdgeOffset + (int) (Y * EdgeSectionSize.Y)), true))
				{
ReturnResult.ProblemAdd("Error: Bad border node.");
				...
				if (SymmetryBlockCountXY.X == 1)
				{
					if (!MakePassageNodes(new modMath.sXY_int(TileSize.X * modProgram.TerrainGridSpacing - EdgeOffset, EdgeOffset + (int) (Y * EdgeSectionSize.Y)), true))
					{
ReturnResult.ProblemAdd("Error: Bad border node.");
						return ReturnResult;
					}
			...
			{
				if (!MakePassageNodes(new modMath.sXY_int(EdgeOffset + (int) (X * EdgeSectionSize.X), EdgeOffset), true))
				{
ReturnResult.ProblemAdd("Error: Bad border node.");
					return ReturnResult;
				}
				if (SymmetryBlockCountXY.Y == 1)
				{
					if (!MakePassageNodes(new modMath.sXY_int(EdgeOffset + (int) (X * EdgeSectionSize.X), TileSize.Y * modProgram.TerrainGridSpacing - EdgeOffset), true))
					{
ReturnResult.ProblemAdd("Error: Bad border node.");
						return ReturnResult;
					}
...
PointMakingFinished:
			PassageNodes = (clsPassageNode[,]) Microsoft.VisualBasic.CompilerServices.Utils.CopyArray((Array) PassageNodes, new clsPassageNode[SymmetryBlockCount - 1 + 1, PassageNodeCount - 1 + 1]);
			
			//connect until all are connected without intersecting
			
			modMath.sIntersectPos IntersectPos = new modMath.sIntersectPos();
						...
						if (NearestB != NearestA)
						{
							...
							if (Flag)
							{
								for (B = 0; B <= NearestB.NodeCount - 1; B++)
								{
									...
									{
										IntersectPos = modMath.GetLinesIntersectBetween(tmpPassageNodeA.Pos, tmpPassageNodeB.Pos, NearestB.NodeA[B].Pos, NearestB.NodeB[B].Pos);
			...
			for (B = 0; B <= TopLeftPlayerCount - 1; B++)
			{
				...
				for (A = 0; A <= SymmetryBlockCount - 1; A++)
				{
					...
					RotatedPos = TileOrientation.GetRotatedPos(SymmetryBlocks[A].Orientation, PlayerBasePos[B], new modMath.sXY_int(SymmetrySize.X - 1, SymmetrySize.Y - 1));
			...
			for (A = 0; A <= PassageNodeCount - 1; A++)
			{
				...
				if (tmpPassageNodeA.Level < 0 && !tmpPassageNodeA.IsOnBorder)
				{
					...
					if (CanDoFlatsAroundWater && ((WaterCount == 0 & WaterSpawns < WaterSpawnQuantity) || (WaterCount == 1 & TotalWaterQuantity - TotalWater > WaterSpawnQuantity - WaterSpawns)) && HeightsArgs.PassageNodesMinLevel.Nodes[tmpPassageNodeA.Num] == 0 & TotalWater < TotalWaterQuantity)
					{
						if (WaterCount == 0)
						{
							WaterSpawns++;
...
ReturnResult.ProblemAdd("Error: Border has had its height set.");
					...
					for (B = 0; B <= tmpPassageNodeA.ConnectionCount - 1; B++)
					{
						...
						{
							if (HeightsArgs.PassageNodesMinLevel.Nodes[tmpPassageNodeA.Num] <= tmpPassageNodeB.Level && HeightsArgs.PassageNodesMaxLevel.Nodes[tmpPassageNodeA.Num] >= tmpPassageNodeB.Level)
							{
		...
		private bool TestNearest(clsTestNearestArgs Args)
		{
			...
			for (A = 0; A <= PassageNodeCount - 1; A++)
			{
				for (B = 0; B <= SymmetryBlockCount - 1; B++)
				{
					...
					{
						XY_int = modMath.PointGetClosestPosOnLine(Args.PassageNodeA.Pos, Args.PassageNodeB.Pos, PassageNodes[B, A].Pos);
		...
		public clsResult GenerateLayoutTerrain()
		{
			...
			for (Y = 0; Y <= Map.Terrain.TileSize.Y; Y++)
			{
				for (X = 0; X <= Map.Terrain.TileSize.X; X++)
				{
					...
					NodeTag.Pos = new modMath.sXY_int(X * 128, Y * 128);
			...
			for (A = 0; A <= JitterLayer.GetNodeCount - 1; A++)
			{
				...
				for (B = 0; B <= ConnectionCount - 1; B++)
				{
					...
					if (Connections[B].PassageNodeA.Level == Connections[B].PassageNodeB.Level)
					{
						...
						{
							BestDist = Dist;
							if ((NodeTag.Pos - Connections[B].PassageNodeA.Pos).ToDoubles().GetMagnitude() <= (NodeTag.Pos - Connections[B].PassageNodeB.Pos).ToDoubles().GetMagnitude())
							{
...
ReturnResult.ProblemAdd("Error: Node height is not set.");
			...
			for (B = 0; B <= ConnectionCount - 1; B++)
			{
				...
				RampArgs.RampLength = Math.Max(System.Convert.ToInt32((Connections[B].PassageNodeA.Pos - Connections[B].PassageNodeB.Pos).ToDoubles().GetMagnitude() * 0.75D), MinRampLength * Math.Abs(Connections[B].PassageNodeA.Level - Connections[B].PassageNodeB.Level));
				for (A = 0; A <= JitterLayer.GetNodeCount - 1; A++)
				{
		...
		public void GenerateTilePathMap()
		{
			...
			for (Y = 0; Y <= Map.Terrain.TileSize.Y - 1; Y++)
			{
				for (X = 0; X <= Map.Terrain.TileSize.X - 1; X++)
				{
					...
					NodeTag = new clsNodeTag();
					NodeTag.Pos = new modMath.sXY_int((int) ((X + 0.5D) * 128.0D), (int) ((Y + 0.5D) * 128.0D));
		...
		private void UpdateNodeConnectedness(clsUpdateNodeConnectednessArgs Args, clsPassageNode PassageNode)
		{
			...
			for (A = 0; A <= PassageNode.ConnectionCount - 1; A++)
			{
				...
				if (!(tmpConnection.PassageNodeA.IsOnBorder || tmpConnection.PassageNodeB.IsOnBorder || tmpConnection.PassageNodeA.IsWater || tmpConnection.PassageNodeB.IsWater) && (tmpConnection.IsRamp || tmpConnection.PassageNodeA.Level == tmpConnection.PassageNodeB.Level))
				{
		...
		private void OilValueCalc(clsOilBalanceLoopArgs Args)
		{
			...
			for (NewOilNum = 0; NewOilNum <= OilAtATime - 1; NewOilNum++)
			{
				...
				for (OtherOilNum = NewOilNum + 1; OtherOilNum <= OilAtATime - 1; OtherOilNum++)
				{
					...
					OilDistValue = 4.0D * PassageNodeDists[0, NewOilNodeNum, 0, OtherOilNodeNum]; //+ GetDist_XY_int(PassageNodes(0, NewOilNodeNum).Pos, PassageNodes(0, OtherOilNodeNum).Pos))
				...
				for (OtherOilNum = 0; OtherOilNum <= OilAtATime - 1; OtherOilNum++)
				{
					...
					for (SymmetryBlockNum = 1; SymmetryBlockNum <= SymmetryBlockCount - 1; SymmetryBlockNum++)
					{
						...
						OilDistValue = 4.0D * PassageNodeDists[0, NewOilNodeNum, SymmetryBlockNum, OtherOilNodeNum]; // + GetDist_XY_int(PassageNodes(0, NewOilNodeNum).Pos, PassageNodes(SymmetryBlockNum, OtherOilNodeNum).Pos))
						if (OilDistValue < NearestOilValue)
						{
							NearestOilValue = OilDistValue;
						}
				...
				for (MapNodeNum = 0; MapNodeNum <= PassageNodeCount - 1; MapNodeNum++)
				{
					for (SymmetryBlockNum = 0; SymmetryBlockNum <= SymmetryBlockCount - 1; SymmetryBlockNum++)
					{
						if (PassageNodes[SymmetryBlockNum, MapNodeNum].OilCount > 0)
						{
							...
							OilDistValue = 4.0D * OilMassMultiplier / PassageNodeDists[0, NewOilNodeNum, SymmetryBlockNum, MapNodeNum]; // + GetDist_XY_int(PassageNodes(0, NewOilNodeNum).Pos, PassageNodes(SymmetryBlockNum, MapNodeNum).Pos))
							if (OilDistValue < NearestOilValue)
							{
								NearestOilValue = OilDistValue;
							}
				...
				for (PlayerNum = 0; PlayerNum <= TopLeftPlayerCount - 1; PlayerNum++)
				{
					...
					for (SymmetryBlockNum = 0; SymmetryBlockNum <= SymmetryBlockCount - 1; SymmetryBlockNum++)
					{
						dblTemp = System.Convert.ToDouble(PassageNodeDists[0, PlayerBases[PlayerNum].Nodes[0].Num, SymmetryBlockNum, NewOilNodeNum] * 2.0D + (PlayerBases[PlayerNum].Nodes[0].Pos - PassageNodes[SymmetryBlockNum, NewOilNodeNum].Pos).ToDoubles().GetMagnitude());
		...
		public void TerrainBlockPaths()
		{
			...
			for (Y = 0; Y <= Map.Terrain.TileSize.Y - 1; Y++)
			{
				for (X = 0; X <= Map.Terrain.TileSize.X - 1; X++)
				{
					...
					{
						if (GenerateTileset.Tileset.Tiles[Map.Terrain.Tiles[X, Y].Texture.TextureNum].Default_Type == modProgram.TileTypeNum_Cliff || GenerateTileset.Tileset.Tiles[Map.Terrain.Tiles[X, Y].Texture.TextureNum].Default_Type == modProgram.TileTypeNum_Water)
						{
							TileNodeBlock(X, Y);
						}
					}
		...
		public clsBooleanMap GetWaterMap()
		{
			...
			for (Y = 0; Y <= Map.Terrain.TileSize.Y; Y++)
			{
				...
				{
					BestDist = float.MaxValue;
					Pos = new modMath.sXY_int(X * modProgram.TerrainGridSpacing, Y * modProgram.TerrainGridSpacing);
					for (B = 0; B <= ConnectionCount - 1; B++)
					{
						//If Not (Connections(B).PassageNodeA.IsOnBorder Or Connections(B).PassageNodeB.IsOnBorder) Then
						if (Connections[B].PassageNodeA.IsWater == Connections[B].PassageNodeB.IsWater)
						{
							...
							{
								BestDist = Dist;
								if ((Pos - Connections[B].PassageNodeA.Pos).ToDoubles().GetMagnitude() <= (Pos - Connections[B].PassageNodeB.Pos).ToDoubles().GetMagnitude())
								{
		...
		public clsMap.clsUnit PlaceUnitNear(clsUnitType Type, modMath.sXY_int Pos, clsMap.clsUnitGroup UnitGroup, int Clearance, int Rotation, int MaxDistFromPos)
		{
			...
			clsNodeTag NodeTag = default(clsNodeTag);
			modMath.sXY_int FinalTilePos = new modMath.sXY_int();
			modMath.sXY_int TilePosA = new modMath.sXY_int();
			modMath.sXY_int TilePosB = new modMath.sXY_int();
			int X2 = 0;
			int Y2 = 0;
			int Remainder = 0;
			...
			if (PosNode != null)
			{
				...
				if ((Pos - NodeTag.Pos).ToDoubles().GetMagnitude() <= MaxDistFromPos)
				{
					...
					FinalTilePos.X = (int) (Conversion.Int(NodeTag.Pos.X / modProgram.TerrainGridSpacing));
					FinalTilePos.Y = Conversion.Int(NodeTag.Pos.Y / modProgram.TerrainGridSpacing);
					...
					else
					{
						if (VBMath.Rnd() >= 0.5F)
						{
							NewUnit.Pos.Horizontal.X = (int)(NodeTag.Pos.X - modProgram.TerrainGridSpacing / 2.0D);
						}
						else
						{
							NewUnit.Pos.Horizontal.X = (int)(NodeTag.Pos.X + modProgram.TerrainGridSpacing / 2.0D);
					...
					else
					{
						if (VBMath.Rnd() >= 0.5F)
						{
							NewUnit.Pos.Horizontal.Y = (int)(NodeTag.Pos.Y - modProgram.TerrainGridSpacing / 2.0D);
						}
						else
						{
							NewUnit.Pos.Horizontal.Y = (int)(NodeTag.Pos.Y + modProgram.TerrainGridSpacing / 2.0D);
					...
					TilePosA.X = (int)Conversion.Int(NewUnit.Pos.Horizontal.X / modProgram.TerrainGridSpacing - Footprint.X / 2.0D + 0.5D);
					TilePosA.Y = (int)Conversion.Int(NewUnit.Pos.Horizontal.Y / modProgram.TerrainGridSpacing - Footprint.Y / 2.0D + 0.5D);
					TilePosB.X = (int) (Conversion.Int(NewUnit.Pos.Horizontal.X / modProgram.TerrainGridSpacing + Footprint.X / 2.0D - 0.5D));
					TilePosB.Y = (int) (Conversion.Int(NewUnit.Pos.Horizontal.Y / modProgram.TerrainGridSpacing + Footprint.Y / 2.0D - 0.5D));
		...
		public clsMap.clsUnit PlaceUnit(clsUnitType Type, modProgram.sWorldPos Pos, clsMap.clsUnitGroup UnitGroup, int Rotation)
		{
			...
			FinalTilePos.X = (int) (Conversion.Int(Pos.Horizontal.X / modProgram.TerrainGridSpacing));
			FinalTilePos.Y = (int) (Conversion.Int(Pos.Horizontal.Y / modProgram.TerrainGridSpacing));
		...
		public clsResult GenerateUnits()
		{
			...
			modMath.sXY_int TilePos = new modMath.sXY_int();
			...
			for (A = 0; A <= PassageNodeCount - 1; A++)
			{
				for (D = 0; D <= SymmetryBlockCount - 1; D++)
				{
					for (B = 0; B <= PassageNodes[D, A].OilCount - 1; B++)
					{
						...
						TilePos.X = Conversion.Int(tmpUnit.Pos.Horizontal.X / modProgram.TerrainGridSpacing);
						TilePos.Y = (int) (Conversion.Int(tmpUnit.Pos.Horizontal.Y / modProgram.TerrainGridSpacing));
						AverageHeight = (byte) (((Terrain.Vertices[TilePos.X, TilePos.Y].Height) + (Terrain.Vertices[TilePos.X + 1, TilePos.Y].Height) + (Terrain.Vertices[TilePos.X, TilePos.Y + 1].Height) + Terrain.Vertices[TilePos.X + 1, TilePos.Y + 1].Height) / 4.0D);
			...
			if (GenerateTileset.ClusteredUnitChanceTotal > 0)
			{
				for (A = 0; A <= PassageNodeCount - 1; A++)
				{
					for (D = 0; D <= SymmetryBlockCount - 1; D++)
					{
						...
						{
							Count = FeatureClusterMinUnits + System.Convert.ToInt32(Conversion.Int(VBMath.Rnd() * (FeatureClusterMaxUnits - FeatureClusterMinUnits + 1)));
							for (B = 1; B <= Count; B++)
							{
...
ReturnResult.ProblemAdd("Error: bad node count on top layer!");
		...
		public modProgram.sResult GenerateGateways()
		{
			...
			while (PossibleGatewayCount > 0)
			{
				...
				{
					Map.GatewayCreateStoreChange(PossibleGateways[BestNum].StartPos, new modMath.sXY_int(PossibleGateways[BestNum].StartPos.X, PossibleGateways[BestNum].StartPos.Y + PossibleGateways[BestNum].Length - 1));
				...
				{
					Map.GatewayCreateStoreChange(PossibleGateways[BestNum].StartPos, new modMath.sXY_int(PossibleGateways[BestNum].StartPos.X + PossibleGateways[BestNum].Length - 1, PossibleGateways[BestNum].StartPos.Y));
		...
		private bool MakePassageNodes(modMath.sXY_int Pos, bool IsOnBorder)
		{
			...
			modMath.sXY_int RotatedPos = new modMath.sXY_int();
			modMath.sXY_int SymmetrySize = new modMath.sXY_int();
			modMath.sXY_int[] Positions = new modMath.sXY_int[4];
			modMath.sXY_int Limits = new modMath.sXY_int();
			...
			for (A = 0; A <= SymmetryBlockCount - 1; A++)
			{
				...
				for (B = 0; B <= A - 1; B++)
				{
					if ((Positions[A] - Positions[B]).ToDoubles().GetMagnitude() < NodeScale * modProgram.TerrainGridSpacing * 2.0D)
					{
						return false;
					}
				}
		...
		private bool CheckRampAngles(clsConnection NewRampConnection, double MinSpacingAngle, double MinSpacingAngle2, double MinPassageSpacingAngle)
		{
			...
			if (NewRampConnection.PassageNodeA.Connections[NewRampConnection.PassageNodeA_ConnectionNum].IsB)
			{
				...
				NodeAAwayAngle = modMath.AngleClamp(NodeBAwayAngle - Math.PI);
			}
			else
			{
				NodeAAwayAngle = XY_int.ToDoubles().GetAngle();
				NodeBAwayAngle = modMath.AngleClamp(NodeBAwayAngle - Math.PI);
			}
		...
		private bool CheckRampNodeRampAngles(clsPassageNode RampPassageNode, clsPassageNode OtherRampPassageNode, double RampAwayAngle, double MinSpacingAngle, double MinSpacingAngle2)
		{
			...
			for (ConnectionNum = 0; ConnectionNum <= RampPassageNode.ConnectionCount - 1; ConnectionNum++)
			{
				...
				if (tmpConnection.IsRamp)
				{
					...
					SpacingAngle = Math.Abs(modMath.AngleClamp(RampAwayAngle - XY_int.ToDoubles().GetAngle()));
			...
			for (ConnectionNum = 0; ConnectionNum <= RampPassageNode.ConnectionCount - 1; ConnectionNum++)
			{
				...
				if (OtherPassageNode.Level == RampPassageNode.Level)
				{
					...
					if (NarrowConnection)
					{
						...
						//    Return False
						//End If
						if (Math.Abs(modMath.AngleClamp(XY_int.ToDoubles().GetAngle() - RampAwayAngle)) < MinSpacingAngle)
						{
							return false;
						}
		...
		public clsResult GenerateRamps()
		{
			...
			do
			{
				...
				for (B = 0; B <= ConnectionCount - 1; B++)
				{
					...
					{
						if (CheckRampAngles(Connections[B], System.Convert.ToDouble(80.0D * modMath.RadOf1Deg), System.Convert.ToDouble(120.0D * modMath.RadOf1Deg), 0.0D * modMath.RadOf1Deg))
						{
...
Finish:
		...
		public clsResult GenerateOil()
		{
			...
			while (PlacedExtraOilCount < ExtraOilCount)
			{
				...
				for (A = 0; A <= OilAtATime - 1; A++)
				{
					OilArgs.OilClusterSizes[A] = Math.Min(ExtraOilClusterSizeMin + (int) (Conversion.Int(VBMath.Rnd() * (ExtraOilClusterSizeMax - ExtraOilClusterSizeMin + 1))), Math.Max((int) (Math.Ceiling(System.Convert.ToDecimal((ExtraOilCount - PlacedExtraOilCount) / SymmetryBlockCount))), 1));
				}
