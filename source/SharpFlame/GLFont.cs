...
namespace SharpFlame
{
		...
		private void GLTextures_Generate(Font NewBaseFont)
		{
			...
			for (A = 0; A <= 255; A++)
			{
				...
				if (NewSizeX <= 0)
				{
					...
					gfx.Clear(Color.Transparent);
					gfx.Dispose();
					BitmapData = TexBitmap.LockBits(new Rectangle(0, 0, TexBitmap.Width, TexBitmap.Height), System.Drawing.Imaging.ImageLockMode.ReadOnly, System.Drawing.Imaging.PixelFormat.Format32bppArgb);
					...
					OpenTK.Graphics.OpenGL.GL.TexImage2D(TextureTarget.Texture2D, 0, PixelInternalFormat.Rgba, TexBitmap.Width, TexBitmap.Height, 0, PixelFormat.Bgra, PixelType.UnsignedByte, BitmapData.Scan0);
	...
	public class clsTextLabel
	{
		...
		public void Draw()
		{
			...
			int CharCode = 0;
			float CharWidth = 0;
			modMath.sXY_sng TexRatio = new modMath.sXY_sng();
			float LetterPosA = 0;
			float LetterPosB = 0;
			float PosY1 = 0;
			float PosY2 = 0;
