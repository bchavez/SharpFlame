...
namespace SharpFlame
{
	public partial class clsMap
	{
		...
		{
			clsResult ReturnResult = new clsResult("Loading WZ from " + System.Convert.ToString(ControlChars.Quote) + Path + System.Convert.ToString(ControlChars.Quote));
			...
			else
			{
				...
				frmWZLoad SelectToLoadForm = new frmWZLoad(Names, SelectToLoadResult, "Select a map from " + System.Convert.ToString(new modProgram.sSplitPath(Path).FileTitle));
		...
		{
			clsResult ReturnResult = new clsResult("Loading game file from " + System.Convert.ToString(ControlChars.Quote) + Path + System.Convert.ToString(ControlChars.Quote));
			modProgram.sResult SubResult = new modProgram.sResult();
			string Quote = ControlChars.Quote.ToString();
			...
			if (!SubResult.Success)
			{
				MsgBoxResult PromptResult = Interaction.MsgBox("game.map file not found at " + GameFilesPath + ControlChars.NewLine + "Do you want to select another directory to load the underlying map from?", (Microsoft.VisualBasic.MsgBoxStyle) (MsgBoxStyle.OkCancel | MsgBoxStyle.Question), null);
		...
		public clsResult CreateWZObjects(sCreateWZObjectsArgs Args)
		{
			...
			if (INIStructures != null)
			{
				for (A = 0; A <= INIStructures.StructureCount - 1; A++)
				{
					...
					{
						UnitType = modProgram.ObjectData.FindOrCreateUnitType(System.Convert.ToString(INIStructures.Structures[A].Code), clsUnitType.enumType.PlayerStructure, INIStructures.Structures[A].WallType);
		...
		private modProgram.sResult Read_WZ_gam(System.IO.BinaryReader File)
		{
			...
			try
			{
				...
				if (Version != 8U)
				{
					if (Interaction.MsgBox("Game file version is unknown. Continue?", (Microsoft.VisualBasic.MsgBoxStyle) (MsgBoxStyle.OkCancel | MsgBoxStyle.Question), null) != MsgBoxResult.Ok)
					{
		...
		private modProgram.sResult Read_WZ_map(System.IO.BinaryReader File)
		{
			...
			try
			{
				...
				if (Version != 10U)
				{
					if (Interaction.MsgBox("game.map version is unknown. Continue?", (Microsoft.VisualBasic.MsgBoxStyle) (MsgBoxStyle.OkCancel | MsgBoxStyle.Question), null) != MsgBoxResult.Ok)
					{
						ReturnResult.Problem = "Aborted.";
						return ReturnResult;
					}
		...
		private modProgram.sResult Read_WZ_Features(System.IO.BinaryReader File, modLists.SimpleClassList<clsWZBJOUnit> WZUnits)
		{
			...
			try
			{
				...
				if (Version != 8U)
				{
					if (Interaction.MsgBox("feat.bjo version is unknown. Continue?", (Microsoft.VisualBasic.MsgBoxStyle) (MsgBoxStyle.OkCancel | MsgBoxStyle.Question), null) != MsgBoxResult.Ok)
					{
						ReturnResult.Problem = "Aborted.";
						return ReturnResult;
					}
		...
		private modProgram.sResult Read_WZ_TileTypes(System.IO.BinaryReader File)
		{
			...
			try
			{
				...
				if (Version != 8U)
				{
					...
					//Exit Function
					if (Interaction.MsgBox("ttypes.ttp version is unknown. Continue?", (Microsoft.VisualBasic.MsgBoxStyle) (MsgBoxStyle.OkCancel | MsgBoxStyle.Question), null) != MsgBoxResult.Ok)
					{
						ReturnResult.Problem = "Aborted.";
						return ReturnResult;
					}
		...
		private modProgram.sResult Read_WZ_Structures(System.IO.BinaryReader File, modLists.SimpleClassList<clsWZBJOUnit> WZUnits)
		{
			...
			try
			{
				...
				if (Version != 8U)
				{
					if (Interaction.MsgBox("struct.bjo version is unknown. Continue?", (Microsoft.VisualBasic.MsgBoxStyle) (MsgBoxStyle.OkCancel | MsgBoxStyle.Question), null) != MsgBoxResult.Ok)
					{
						ReturnResult.Problem = "Aborted.";
						return ReturnResult;
					}
		...
		private modProgram.sResult Read_WZ_Droids(System.IO.BinaryReader File, modLists.SimpleClassList<clsWZBJOUnit> WZUnits)
		{
			...
			try
			{
				...
				if (Version > 19U)
				{
					if (Interaction.MsgBox("dinit.bjo version is unknown. Continue?", (Microsoft.VisualBasic.MsgBoxStyle) (MsgBoxStyle.OkCancel | MsgBoxStyle.Question), null) != MsgBoxResult.Ok)
					{
						ReturnResult.Problem = "Aborted.";
						return ReturnResult;
					}
			...
			for (A = 0; A <= PriorityOrder.Result.Count - 1; A++)
			{
				...
				else
				{
					...
					if (UnitModuleCount[Unit.MapLink.ArrayPosition] > ModuleLimit)
					{
						...
						{
							ReturnResult.WarningAdd("Structure " + StructureType.GetDisplayTextCode() + " at " + Unit.GetPosText() + " has too many modules (" + System.Convert.ToString(UnitModuleCount[Unit.MapLink.ArrayPosition]) + ").");
						}
		...
		{
			clsResult ReturnResult = new clsResult("Compiling to " + System.Convert.ToString(ControlChars.Quote) + Args.Path + System.Convert.ToString(ControlChars.Quote));
			...
			try
			{
				...
				if (Args.CompileType == sWrite_WZ_Args.enumCompileType.Multiplayer)
				{
					...
					Text = "// Made with " + modProgram.ProgramName + " " + modProgram.ProgramVersionNumber + " " + modProgram.ProgramPlatform + System.Convert.ToString(EndChar);
					File_LEV.Write(Text);
					DateTime DateNow = DateTime.Now;
					Text = "// Date: " + System.Convert.ToString(DateNow.Year) + "/" + modProgram.MinDigits(DateNow.Month, 2) + "/" + modProgram.MinDigits(DateNow.Day, 2) + " " + modProgram.MinDigits(DateNow.Hour, 2) + ":" + modProgram.MinDigits(DateNow.Minute, 2) + ":" + modProgram.MinDigits(DateNow.Second, 2) + System.Convert.ToString(EndChar);
					File_LEV.Write(Text);
					...
					File_LEV.Write(Text);
					Text = "game    " + System.Convert.ToString(Quote) + "multiplay/maps/" + PlayersPrefix + Args.MapName + ".gam" + System.Convert.ToString(Quote) + System.Convert.ToString(EndChar);
					File_LEV.Write(Text);
					Text = "data    " + System.Convert.ToString(Quote) + "wrf/multi/skirmish" + PlayersText + ".wrf" + System.Convert.ToString(Quote) + System.Convert.ToString(EndChar);
					File_LEV.Write(Text);
					...
					Text = "game    " + System.Convert.ToString(Quote) + "multiplay/maps/" + PlayersPrefix + Args.MapName + ".gam" + System.Convert.ToString(Quote) + System.Convert.ToString(EndChar);
					File_LEV.Write(Text);
					Text = "data    " + System.Convert.ToString(Quote) + "wrf/multi/t2-skirmish" + PlayersText + ".wrf" + System.Convert.ToString(Quote) + System.Convert.ToString(EndChar);
					File_LEV.Write(Text);
					Text = "data    " + System.Convert.ToString(Quote) + "wrf/multi/" + fog + System.Convert.ToString(Quote) + System.Convert.ToString(EndChar);
					File_LEV.Write(Text);
					Text = EndChar.ToString();
					...
					Text = "game    " + System.Convert.ToString(Quote) + "multiplay/maps/" + PlayersPrefix + Args.MapName + ".gam" + System.Convert.ToString(Quote) + System.Convert.ToString(EndChar);
					File_LEV.Write(Text);
					Text = "data    " + System.Convert.ToString(Quote) + "wrf/multi/t3-skirmish" + PlayersText + ".wrf" + System.Convert.ToString(Quote) + System.Convert.ToString(EndChar);
					File_LEV.Write(Text);
					Text = "data    " + System.Convert.ToString(Quote) + "wrf/multi/" + fog + System.Convert.ToString(Quote) + System.Convert.ToString(EndChar);
					File_LEV.Write(Text);
				}
				...
				for (Y = 0; Y <= Terrain.TileSize.Y - 1; Y++)
				{
					for (X = 0; X <= Terrain.TileSize.X - 1; X++)
					{
						...
						if (TextureNum < 0 | TextureNum > 255)
						{
							...
							{
								ReturnResult.WarningAdd("Tile texture number " + System.Convert.ToString(Terrain.Tiles[X, Y].Texture.TextureNum) + " is invalid on tile " + System.Convert.ToString(X) + ", " + System.Convert.ToString(Y) + " and was compiled as texture number " + System.Convert.ToString(TextureNum) + ".");
							}
							InvalidTileCount++;
						}
