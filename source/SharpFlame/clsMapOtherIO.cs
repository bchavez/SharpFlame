...
namespace SharpFlame
{
	public partial class clsMap
	{
		...
		{
			clsResult ReturnResult = new clsResult("Loading FME from " + System.Convert.ToString(ControlChars.Quote) + Path + System.Convert.ToString(ControlChars.Quote));
		...
		{
			clsResult ReturnResult = new clsResult("Loading LND from " + System.Convert.ToString(ControlChars.Quote) + Path + System.Convert.ToString(ControlChars.Quote));
...
LineDone:
		...
		{
			clsResult ReturnResult = new clsResult("Writing LND to " + System.Convert.ToString(ControlChars.Quote) + Path + System.Convert.ToString(ControlChars.Quote));
			...
			try
			{
				...
				for (Y = 0; Y <= Terrain.TileSize.Y - 1; Y++)
				{
					for (X = 0; X <= Terrain.TileSize.X - 1; X++)
					{
						...
						Text = "        TID " + (Terrain.Tiles[X, Y].Texture.TextureNum + 1) + " VF " + modIO.InvariantToString_int(VF) + " TF " + modIO.InvariantToString_int(TF) + " F " + modIO.InvariantToString_int(Flip) + " VH " + modIO.InvariantToString_byte(System.Convert.ToByte(Terrain.Vertices[X, Y].Height)) + " " + modIO.InvariantToString_byte(Terrain.Vertices[X + 1, Y].Height) + " " + System.Convert.ToString(Terrain.Vertices[X + 1, Y + 1].Height) + " " + modIO.InvariantToString_byte(System.Convert.ToByte(Terrain.Vertices[X, Y + 1].Height)) + System.Convert.ToString(EndChar);
						File.Write(Text);
					}
				}
				Text = "    }" + System.Convert.ToString(EndChar);
				...
				foreach (clsMap.clsUnit tempLoopVar_Unit in Units)
				{
					...
					if (B >= 0)
					{
						...
						{
							Text = "        " + modIO.InvariantToString_uint(Unit.ID) + " " + System.Convert.ToString(B) + " " + System.Convert.ToString(Quote) + Code + System.Convert.ToString(Quote) + " " + Unit.UnitGroup.GetLNDPlayerText() + " " + System.Convert.ToString(Quote) + "NONAME" + System.Convert.ToString(Quote) + " " + modIO.InvariantToString_int(XYZ_int.X) + ".00 " + modIO.InvariantToString_int(XYZ_int.Y) + ".00 " + modIO.InvariantToString_int(XYZ_int.Z) + ".00 0.00 " + modIO.InvariantToString_int(Unit.Rotation) + ".00 0.00" + System.Convert.ToString(EndChar);
							File.Write(Text);
						}
						else
						{
				...
				File.Write(Text);
				Text = "        " + System.Convert.ToString(Quote) + "Entire Map" + System.Convert.ToString(Quote) + " 0 0 0 " + modIO.InvariantToString_int(Terrain.TileSize.X) + " " + modIO.InvariantToString_int(Terrain.TileSize.Y) + System.Convert.ToString(EndChar);
				File.Write(Text);
				Text = "    }" + System.Convert.ToString(EndChar);
				File.Write(Text);
				Text = "}" + System.Convert.ToString(EndChar);
				...
				{
					Gateway = tempLoopVar_Gateway;
					Text = "        " + modIO.InvariantToString_int(Gateway.PosA.X) + " " + modIO.InvariantToString_int(Gateway.PosA.Y) + " " + modIO.InvariantToString_int(Gateway.PosB.X) + " " + modIO.InvariantToString_int(Gateway.PosB.Y) + System.Convert.ToString(EndChar);
					File.Write(Text);
				}
				Text = "    }" + System.Convert.ToString(EndChar);
				File.Write(Text);
				...
				Text = "    Tiles {" + System.Convert.ToString(EndChar);
				File.Write(Text);
				for (A = 0; A <= ((int) (Math.Ceiling(System.Convert.ToDecimal((Tileset.TileCount + 1) / 16.0D)))) - 1; A++) //+1 because the first number is not a tile type
				{
					Text = "        ";
		...
		public modProgram.sResult Write_MinimapFile(string Path, bool Overwrite)
		{
			...
			for (Y = 0; Y <= Terrain.TileSize.Y - 1; Y++)
			{
				for (X = 0; X <= Terrain.TileSize.X - 1; X++)
				{
					MinimapBitmap.SetPixel(X, Y, System.Drawing.ColorTranslator.FromOle(modColour.OSRGB((int) (modMath.Clamp_sng(System.Convert.ToSingle(Texture.get_Pixels(X, Y).Red * 255.0F), 0.0F, 255.0F)), (int) (modMath.Clamp_sng(System.Convert.ToSingle(Texture.get_Pixels(X, Y).Green * 255.0F), 0.0F, 255.0F)), (int) (modMath.Clamp_sng(System.Convert.ToSingle(Texture.get_Pixels(X, Y).Blue * 255.0F), 0.0F, 255.0F)))));
				}
			}
		...
		public modProgram.sResult Write_Heightmap(string Path, bool Overwrite)
		{
			...
			for (Y = 0; Y <= Terrain.TileSize.Y; Y++)
			{
				for (X = 0; X <= Terrain.TileSize.X; X++)
				{
					HeightmapBitmap.SetPixel(X, Y, System.Drawing.ColorTranslator.FromOle(modColour.OSRGB(System.Convert.ToInt32(Terrain.Vertices[X, Y].Height), Terrain.Vertices[X, Y].Height, Terrain.Vertices[X, Y].Height)));
				}
			}
