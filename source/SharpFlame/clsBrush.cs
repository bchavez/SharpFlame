...
public enumShape Shape
		...
		private void PerformAction(clsMap.clsAction Action, sPosNum PosNum, modMath.sXY_int LastValidNum)
		{
			...
			Action.Effect = 1.0D;
			for (Y = modMath.Clamp_int(Tiles.YMin + Centre.Y, 0, LastValidNum.Y) - Centre.Y; Y <= modMath.Clamp_int(Tiles.YMax + Centre.Y, 0, LastValidNum.Y) - Centre.Y; Y++)
			{
				Action.PosNum.Y = Centre.Y + Y;
				XNum = Y - Tiles.YMin;
				for (X = modMath.Clamp_int(Tiles.XMin[XNum] + Centre.X, 0, LastValidNum.X) - Centre.X; X <= modMath.Clamp_int(System.Convert.ToInt32(Tiles.XMax[XNum] + Centre.X), 0, LastValidNum.X) - Centre.X; X++)
				{
						...
						if (Tiles.ResultRadius > 0.0D)
						{
							switch (_Shape)
							{
								case clsBrush.enumShape.Circle:
									if (_Alignment)
									{
										Action.Effect = System.Convert.ToDouble(1.0D - (new Matrix3D.Position.XY_dbl(Action.PosNum.X, Action.PosNum.Y) - new Matrix3D.Position.XY_dbl(Centre.X - 0.5D, Centre.Y - 0.5D)).GetMagnitude() / (Tiles.ResultRadius + 0.5D));
									}
									else
									{
								...
								case clsBrush.enumShape.Square:
									if (_Alignment)
									{
										Action.Effect = 1.0D - Math.Max(Math.Abs(Action.PosNum.X - (Centre.X - 0.5D)), Math.Abs(Action.PosNum.Y - (Centre.Y - 0.5D))) / (Tiles.ResultRadius + 0.5D);
									}
									else
									{
										Action.Effect = 1.0D - Math.Max(Math.Abs(Action.PosNum.X - Centre.X), Math.Abs(Action.PosNum.Y - Centre.Y)) / (Tiles.ResultRadius + 0.5D);
									}
									break;
							}
						}
