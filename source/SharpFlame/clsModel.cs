					...
					do
					{
						...
						if (char.Parse(strTemp2) == '\t' || strTemp2 == " ")
						{
							...
							if (PIEVersion == 3)
							{
								...
								{
									ReturnResult.ProblemAdd("Wrong number of fields (" + System.Convert.ToString(SplitText.GetUpperBound(0) + 1) + ") for polygon " + System.Convert.ToString(A));
									return ReturnResult;
								}
								for (B = 0; B <= Count - 1; B++)
								{
							...
							else if (PIEVersion == 2)
							{
								...
								do
								{
									...
									if (SplitText[D] == "4200" || SplitText[D] == "4000" || SplitText[D] == "6a00" || SplitText[D] == "4a00" || SplitText[D] == "6200" || SplitText[D] == "14200" || SplitText[D] == "14a00" || SplitText[D] == "16a00")
									{
										C += 4;
									}
									for (B = 0; B <= Levels[LevelNum].Polygon[A].PointCount - 1; B++)
...
FileFinished:
			...
			if (GLTextureNum == 0)
			{
				ReturnResult.WarningAdd("Texture " + System.Convert.ToString(ControlChars.Quote) + TextureName + System.Convert.ToString(ControlChars.Quote) + " was not loaded");
			}
