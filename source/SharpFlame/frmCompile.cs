	...
	public partial class frmCompile
	{
		...
		public void btnCompile_Click(System.Object sender, System.EventArgs e)
		{
			...
			for (A = 0; A <= MapName.Length - 1; A++)
			{
				...
				if (!((CurrentChar >= 'a' && CurrentChar <= 'z') || (CurrentChar >= 'A' && CurrentChar <= 'Z') || (A >= 1 && ((CurrentChar >= '0' && CurrentChar <= '9') || CurrentChar == '-' || CurrentChar == '_'))))
				{
					break;
				}
			}
		...
		private clsResult ValidateMap_UnitPositions()
		{
			...
			foreach (clsMap.clsUnit tempLoopVar_Unit in Map.Units)
			{
				...
				if (Unit.Type.Type == clsUnitType.enumType.PlayerStructure)
				{
					...
					UnitIsStructureModule[Unit.MapLink.ArrayPosition] = StructureType.IsModule() | StructureTypeType == clsStructureType.enumStructureType.ResourceExtractor;
				}
			}
		...
		private clsResult ValidateMap()
		{
			...
			foreach (clsStructureType tempLoopVar_StructureType in modProgram.ObjectData.StructureTypes)
			{
				...
				for (PlayerNum = 0; PlayerNum <= modProgram.PlayerCountMax - 1; PlayerNum++)
				{
					...
					{
						ReturnResult.ProblemAdd("Player " + System.Convert.ToString(PlayerNum) + " has too many (" + System.Convert.ToString(PlayerStructureTypeCount[PlayerNum, StructureTypeNum]) + ") of structure " + System.Convert.ToString(ControlChars.Quote) + StructureType.Code + System.Convert.ToString(ControlChars.Quote) + ". The limit is 255 of any one structure type.");
					}
				}
				if (ScavStructureTypeCount[StructureTypeNum] > 255)
				{
					ReturnResult.ProblemAdd("Scavengers have too many (" + System.Convert.ToString(ScavStructureTypeCount[StructureTypeNum]) + ") of structure " + System.Convert.ToString(ControlChars.Quote) + StructureType.Code + System.Convert.ToString(ControlChars.Quote) + ". The limit is 255 of any one structure type.");
				}
			}
