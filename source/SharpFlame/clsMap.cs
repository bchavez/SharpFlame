...
public bool IsFMap
		...
		public clsMap(clsMap MapToCopy, modMath.sXY_int Offset, modMath.sXY_int Area)
		{
			...
			foreach (clsGateway tempLoopVar_Gateway in MapToCopy.Gateways)
			{
				...
				GatewayCreate(new modMath.sXY_int(Gateway.PosA.X - Offset.X, Gateway.PosA.Y - Offset.Y), new modMath.sXY_int(Gateway.PosB.X - Offset.X, Gateway.PosB.Y - Offset.Y));
			}
			...
			if (modMain.frmMainInstance.menuMiniShowTex.Checked)
			{
				...
				if (modMain.frmMainInstance.menuMiniShowHeight.Checked)
				{
					...
					for (Y = 0; Y <= Terrain.TileSize.Y - 1; Y++)
					{
						for (X = 0; X <= Terrain.TileSize.X - 1; X++)
						{
							Height = System.Convert.ToSingle(((Terrain.Vertices[X, Y].Height) + Terrain.Vertices[X + 1, Y].Height + Terrain.Vertices[X, Y + 1].Height + Terrain.Vertices[X + 1, Y + 1].Height) / 1020.0F);
		...
		private void MinimapMake()
		{
			...
			int NewTextureSize = (int) (Math.Round(System.Convert.ToDouble(Math.Pow(2.0D, Math.Ceiling(Math.Log(Math.Max(Terrain.TileSize.X, Terrain.TileSize.Y)) / Math.Log(2.0D))))));
			...
			GL.TexImage2D<sRGBA_sng>(TextureTarget.Texture2D, 0, PixelInternalFormat.Rgba, Minimap_Texture_Size, Minimap_Texture_Size, 0, PixelFormat.Rgba, PixelType.Float, Texture.InlinePixels);
		...
		public modProgram.sWorldPos TileAlignedPosFromMapPos(modMath.sXY_int Horizontal, modMath.sXY_int Footprint)
		{
			...
			Result.Horizontal.X = (int) ((Math.Round(System.Convert.ToDouble((Horizontal.X - Footprint.X * modProgram.TerrainGridSpacing / 2.0D) / modProgram.TerrainGridSpacing)) + Footprint.X / 2.0D) * modProgram.TerrainGridSpacing);
			Result.Horizontal.Y = (int) ((Math.Round(System.Convert.ToDouble((Horizontal.Y - Footprint.Y * modProgram.TerrainGridSpacing / 2.0D) / modProgram.TerrainGridSpacing)) + Footprint.Y / 2.0D) * modProgram.TerrainGridSpacing);
			Result.Altitude = (int) (GetTerrainHeight(Result.Horizontal));
		...
		public void AutoSaveTest()
		{
			...
			if (DateAndTime.DateDiff("s", AutoSave.SavedDate, DateTime.Now, (Microsoft.VisualBasic.FirstDayOfWeek) Microsoft.VisualBasic.FirstDayOfWeek.Sunday, (Microsoft.VisualBasic.FirstWeekOfYear) Microsoft.VisualBasic.FirstWeekOfYear.Jan1) < modSettings.Settings.AutoSaveMinInterval_s)
			{
				return;
			}
		...
		public clsResult AutoSavePerform()
		{
			...
			if (!System.IO.Directory.Exists(modProgram.AutoSavePath))
			{
				...
				{
					ReturnResult.ProblemAdd("Unable to create directory at " + System.Convert.ToString(ControlChars.Quote) + modProgram.AutoSavePath + System.Convert.ToString(ControlChars.Quote));
				}
			}
			...
			Path = modProgram.AutoSavePath + "autosaved-" + modIO.InvariantToString_int(DateNow.Year) + "-" + modProgram.MinDigits(DateNow.Month, 2) + "-" + modProgram.MinDigits(DateNow.Day, 2) + "-" + modProgram.MinDigits(DateNow.Hour, 2) + "-" + modProgram.MinDigits(DateNow.Minute, 2) + "-" + modProgram.MinDigits(DateNow.Second, 2) + "-" + modProgram.MinDigits(DateNow.Millisecond, 3) + ".fmap";
					...
					if (Terrain.Tiles[PosNum.X, PosNum.Y].TriTopLeftIsCliff)
					{
						if (Terrain.Tiles[PosNum.X, PosNum.Y].TriBottomRightIsCliff)
						{
							...
							for (BrushNum = 0; BrushNum <= Painter.CliffBrushCount - 1; BrushNum++)
							{
								...
								if (((Terrain.Vertices[PosNum.X, PosNum.Y].Terrain == Terrain_Inner && Terrain.Vertices[PosNum.X + 1, PosNum.Y].Terrain == Terrain_Inner) && (Terrain.Vertices[PosNum.X, PosNum.Y + 1].Terrain == Terrain_Outer || Terrain.Vertices[PosNum.X + 1, PosNum.Y + 1].Terrain == Terrain_Outer)) || ((Terrain.Vertices[PosNum.X, PosNum.Y].Terrain == Terrain_Inner || Terrain.Vertices[PosNum.X + 1, PosNum.Y].Terrain == Terrain_Inner) && (Terrain.Vertices[PosNum.X, PosNum.Y + 1].Terrain == Terrain_Outer && Terrain.Vertices[PosNum.X + 1, PosNum.Y + 1].Terrain == Terrain_Outer)))
								{
									ResultTiles = Painter.CliffBrushes[BrushNum].Tiles_Straight;
									ResultDirection = TileOrientation.TileDirection_Bottom;
									break;
								}
								else if (((Terrain.Vertices[PosNum.X, PosNum.Y].Terrain == Terrain_Outer && Terrain.Vertices[PosNum.X, PosNum.Y + 1].Terrain == Terrain_Outer) && (Terrain.Vertices[PosNum.X + 1, PosNum.Y].Terrain == Terrain_Inner || Terrain.Vertices[PosNum.X + 1, PosNum.Y + 1].Terrain == Terrain_Inner)) || ((Terrain.Vertices[PosNum.X, PosNum.Y].Terrain == Terrain_Outer || Terrain.Vertices[PosNum.X, PosNum.Y + 1].Terrain == Terrain_Outer) && (Terrain.Vertices[PosNum.X + 1, PosNum.Y].Terrain == Terrain_Inner && Terrain.Vertices[PosNum.X + 1, PosNum.Y + 1].Terrain == Terrain_Inner)))
								{
									ResultTiles = Painter.CliffBrushes[BrushNum].Tiles_Straight;
									ResultDirection = TileOrientation.TileDirection_Left;
									break;
								}
								else if (((Terrain.Vertices[PosNum.X, PosNum.Y].Terrain == Terrain_Outer && Terrain.Vertices[PosNum.X + 1, PosNum.Y].Terrain == Terrain_Outer) && (Terrain.Vertices[PosNum.X, PosNum.Y + 1].Terrain == Terrain_Inner || Terrain.Vertices[PosNum.X + 1, PosNum.Y + 1].Terrain == Terrain_Inner)) || ((Terrain.Vertices[PosNum.X, PosNum.Y].Terrain == Terrain_Outer || Terrain.Vertices[PosNum.X + 1, PosNum.Y].Terrain == Terrain_Outer) && (Terrain.Vertices[PosNum.X, PosNum.Y + 1].Terrain == Terrain_Inner && Terrain.Vertices[PosNum.X + 1, PosNum.Y + 1].Terrain == Terrain_Inner)))
								{
									ResultTiles = Painter.CliffBrushes[BrushNum].Tiles_Straight;
									ResultDirection = TileOrientation.TileDirection_Top;
									break;
								}
								else if (((Terrain.Vertices[PosNum.X, PosNum.Y].Terrain == Terrain_Inner && Terrain.Vertices[PosNum.X, PosNum.Y + 1].Terrain == Terrain_Inner) && (Terrain.Vertices[PosNum.X + 1, PosNum.Y].Terrain == Terrain_Outer || Terrain.Vertices[PosNum.X + 1, PosNum.Y + 1].Terrain == Terrain_Outer)) || ((Terrain.Vertices[PosNum.X, PosNum.Y].Terrain == Terrain_Inner || Terrain.Vertices[PosNum.X, PosNum.Y + 1].Terrain == Terrain_Inner) && (Terrain.Vertices[PosNum.X + 1, PosNum.Y].Terrain == Terrain_Outer && Terrain.Vertices[PosNum.X + 1, PosNum.Y + 1].Terrain == Terrain_Outer)))
								{
									ResultTiles = Painter.CliffBrushes[BrushNum].Tiles_Straight;
									ResultDirection = TileOrientation.TileDirection_Right;
									break;
		...
		public bool CheckMessages()
		{
			...
			{
				if (DateAndTime.DateDiff(DateInterval.Second, System.Convert.ToDateTime(Messages[A].CreatedDate), DateNow, (Microsoft.VisualBasic.FirstDayOfWeek) Microsoft.VisualBasic.FirstDayOfWeek.Sunday, (Microsoft.VisualBasic.FirstWeekOfYear) Microsoft.VisualBasic.FirstWeekOfYear.Jan1) >= 6)
				{
					Messages.Remove(A);
					Changed = true;
				}
