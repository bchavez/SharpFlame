...
namespace SharpFlame
{
	public class clsViewInfo
	{
		...
		public Matrix3D.Angles.AngleRPY ViewAngleRPY;
		...
		public clsViewInfo(clsMap Map, ctrlMapView MapView)
		{
			...
			ViewPos = new modMath.sXYZ_int(0, 3072, 0);
			FOV_Multiplier_Set(modSettings.Settings.FOVDefault);
			ViewAngleSetToDefault();
			LookAtPos(new modMath.sXY_int((int) (Map.Terrain.TileSize.X * modProgram.TerrainGridSpacing / 2.0D), (int) (Map.Terrain.TileSize.Y * modProgram.TerrainGridSpacing / 2.0D)));
		}
		...
		{
			const float Min = (float)(0.1d * modMath.RadOf1Deg);
			const float Max = (float)(179.0d * modMath.RadOf1Deg);
		...
		private void ViewPosClamp()
		{
			...
			ViewPos.X = modMath.Clamp_int(ViewPos.X, System.Convert.ToInt32(- MaxDist), Map.Terrain.TileSize.X * modProgram.TerrainGridSpacing + MaxDist);
			ViewPos.Z = modMath.Clamp_int(ViewPos.Z, - Map.Terrain.TileSize.Y * modProgram.TerrainGridSpacing - MaxDist, MaxDist);
			ViewPos.Y = modMath.Clamp_int(ViewPos.Y, ((int) (Math.Ceiling(Map.GetTerrainHeight(new modMath.sXY_int(ViewPos.X, - ViewPos.Z))))) + 16, MaxHeight);
		}
		...
		public void ViewAngleSet_Rotate(Matrix3DMath.Matrix3D NewMatrix)
		{
			...
			if (modProgram.ViewMoveType == modProgram.enumView_Move_Type.RTS & modProgram.RTSOrbit)
			{
				...
				//    XYZ_dbl.Z = XYZ_lng.Z
				//Else
				if (ScreenXY_Get_ViewPlanePos_ForwardDownOnly((int) (Conversion.Int(MapView.GLSize.X / 2.0D)), (int) (Conversion.Int(MapView.GLSize.Y / 2.0D)), 127.5D, XY_dbl))
				{
					XYZ_dbl.X = XY_dbl.X;
					XYZ_dbl.Y = 127.5D;
					XYZ_dbl.Z = System.Convert.ToDouble(- XY_dbl.Y);
				}
				else
				{
					Flag = false;
			...
			if (Flag)
			{
				if (ViewAngleRPY.Pitch < modMath.RadOf1Deg * 10.0D)
				{
					ViewAngleRPY.Pitch = modMath.RadOf1Deg * 10.0D;
				}
			}
			...
			if (Flag)
			{
				...
				XYZ_dbl2.Z = System.Convert.ToDouble(- ViewPos.Z);
				MoveToViewTerrainPosFromDistance(XYZ_dbl, System.Convert.ToDouble((XYZ_dbl2 - XYZ_dbl).GetMagnitude()));
			}
		...
		public void MoveToViewTerrainPosFromDistance(Matrix3D.Position.XYZ_dbl TerrainPos, double Distance)
		{
			Matrix3D.Position.XYZ_dbl XYZ_dbl = default(Matrix3D.Position.XYZ_dbl);
			modMath.sXYZ_int XYZ_int = new modMath.sXYZ_int();
		...
		public bool Pos_Get_Screen_XY(Matrix3D.Position.XYZ_dbl Pos, modMath.sXY_int Result)
		{
		...
		public bool ScreenXY_Get_ViewPlanePos(modMath.sXY_int ScreenPos, double PlaneHeight, Matrix3D.Position.XY_dbl ResultPos)
		{
			double dblTemp = 0;
			Matrix3D.Position.XYZ_dbl XYZ_dbl = default(Matrix3D.Position.XYZ_dbl);
			Matrix3D.Position.XYZ_dbl XYZ_dbl2 = default(Matrix3D.Position.XYZ_dbl);
		...
		public bool ScreenXY_Get_TerrainPos(modMath.sXY_int ScreenPos, modProgram.sWorldPos ResultPos)
		{
			double dblTemp = 0;
			Matrix3D.Position.XYZ_dbl XYZ_dbl = default(Matrix3D.Position.XYZ_dbl);
			Matrix3D.Position.XYZ_dbl TerrainViewVector = default(Matrix3D.Position.XYZ_dbl);
			int X = 0;
			int Y = 0;
			Matrix3D.Position.XY_dbl LimitA = default(Matrix3D.Position.XY_dbl);
			Matrix3D.Position.XY_dbl LimitB = default(Matrix3D.Position.XY_dbl);
			modMath.sXY_int Min = new modMath.sXY_int();
			modMath.sXY_int Max = new modMath.sXY_int();
			double TriGradientX = 0;
			double TriGradientZ = 0;
			double TriHeightOffset = 0;
			double Dist = 0;
			Matrix3D.Position.XYZ_dbl BestPos = default(Matrix3D.Position.XYZ_dbl);
			double BestDist = 0;
			Matrix3D.Position.XYZ_dbl Dif = default(Matrix3D.Position.XYZ_dbl);
			double InTileX = 0;
			double InTileZ = 0;
			Matrix3D.Position.XY_dbl TilePos = default(Matrix3D.Position.XY_dbl);
			Matrix3D.Position.XYZ_dbl TerrainViewPos = default(Matrix3D.Position.XYZ_dbl);
			...
			try
			{
				...
				TerrainViewPos.Z = System.Convert.ToDouble(- ViewPos.Z);
				...
				TerrainViewVector.Y = System.Convert.ToDouble(- TerrainViewVector.Y); //get the amount of looking down, not up
				TerrainViewVector.Z = System.Convert.ToDouble(- TerrainViewVector.Z); //convert to terrain coordinates from view coordinates
				...
				Min.X = Math.Max(System.Convert.ToInt32(Conversion.Int(Math.Min(LimitA.X, LimitB.X) / modProgram.TerrainGridSpacing)), 0);
				Min.Y = Math.Max((int) (Conversion.Int(Math.Min(LimitA.Y, LimitB.Y) / modProgram.TerrainGridSpacing)), 0);
				Max.X = Math.Min(System.Convert.ToInt32(Conversion.Int(Math.Max(LimitA.X, LimitB.X) / modProgram.TerrainGridSpacing)), Map.Terrain.TileSize.X - 1);
				Max.Y = Math.Min(System.Convert.ToInt32(Conversion.Int(Math.Max(LimitA.Y, LimitB.Y) / modProgram.TerrainGridSpacing)), Map.Terrain.TileSize.Y - 1);
				...
				for (Y = Min.Y; Y <= Max.Y; Y++)
				{
					for (X = Min.X; X <= Max.X; X++)
					{
						...
						{
							TriHeightOffset = System.Convert.ToDouble(Map.Terrain.Vertices[X, Y].Height * Map.HeightMultiplier);
							TriGradientX = System.Convert.ToDouble(Map.Terrain.Vertices[X + 1, Y].Height * Map.HeightMultiplier - TriHeightOffset);
							TriGradientZ = System.Convert.ToDouble(Map.Terrain.Vertices[X, Y + 1].Height * Map.HeightMultiplier - TriHeightOffset);
							XYZ_dbl.Y = (TriHeightOffset + (TriGradientX * (TerrainViewPos.X - TilePos.X) + TriGradientZ * (TerrainViewPos.Z - TilePos.Y) + (TriGradientX * TerrainViewVector.X + TriGradientZ * TerrainViewVector.Z) * TerrainViewPos.Y / TerrainViewVector.Y) / modProgram.TerrainGridSpacing) / (1.0D + (TriGradientX * TerrainViewVector.X + TriGradientZ * TerrainViewVector.Z) / (TerrainViewVector.Y * modProgram.TerrainGridSpacing));
							...
							TriHeightOffset = System.Convert.ToDouble(Map.Terrain.Vertices[X + 1, Y + 1].Height * Map.HeightMultiplier);
							TriGradientX = System.Convert.ToDouble(Map.Terrain.Vertices[X, Y + 1].Height * Map.HeightMultiplier - TriHeightOffset);
							TriGradientZ = System.Convert.ToDouble(Map.Terrain.Vertices[X + 1, Y].Height * Map.HeightMultiplier - TriHeightOffset);
							XYZ_dbl.Y = (TriHeightOffset + TriGradientX + TriGradientZ + (TriGradientX * (TilePos.X - TerrainViewPos.X) + TriGradientZ * (TilePos.Y - TerrainViewPos.Z) - (TriGradientX * TerrainViewVector.X + TriGradientZ * TerrainViewVector.Z) * TerrainViewPos.Y / TerrainViewVector.Y) / modProgram.TerrainGridSpacing) / (1.0D - (TriGradientX * TerrainViewVector.X + TriGradientZ * TerrainViewVector.Z) / (TerrainViewVector.Y * modProgram.TerrainGridSpacing));
							XYZ_dbl.X = TerrainViewPos.X + TerrainViewVector.X * (TerrainViewPos.Y - XYZ_dbl.Y) / TerrainViewVector.Y;
							XYZ_dbl.Z = TerrainViewPos.Z + TerrainViewVector.Z * (TerrainViewPos.Y - XYZ_dbl.Y) / TerrainViewVector.Y;
							InTileX = XYZ_dbl.X / modProgram.TerrainGridSpacing - X;
							InTileZ = XYZ_dbl.Z / modProgram.TerrainGridSpacing - Y;
						...
						else
						{
							...
							TriHeightOffset = System.Convert.ToDouble(Map.Terrain.Vertices[X + 1, Y].Height * Map.HeightMultiplier);
							TriGradientX = System.Convert.ToDouble(Map.Terrain.Vertices[X, Y].Height * Map.HeightMultiplier - TriHeightOffset);
							TriGradientZ = System.Convert.ToDouble(Map.Terrain.Vertices[X + 1, Y + 1].Height * Map.HeightMultiplier - TriHeightOffset);
							XYZ_dbl.Y = (TriHeightOffset + TriGradientX + (TriGradientX * (TilePos.X - TerrainViewPos.X) + TriGradientZ * (TerrainViewPos.Z - TilePos.Y) - (TriGradientX * TerrainViewVector.X - TriGradientZ * TerrainViewVector.Z) * TerrainViewPos.Y / TerrainViewVector.Y) / modProgram.TerrainGridSpacing) / (1.0D - (TriGradientX * TerrainViewVector.X - TriGradientZ * TerrainViewVector.Z) / (TerrainViewVector.Y * modProgram.TerrainGridSpacing));
							XYZ_dbl.X = TerrainViewPos.X + TerrainViewVector.X * (TerrainViewPos.Y - XYZ_dbl.Y) / TerrainViewVector.Y;
							XYZ_dbl.Z = TerrainViewPos.Z + TerrainViewVector.Z * (TerrainViewPos.Y - XYZ_dbl.Y) / TerrainViewVector.Y;
							InTileX = XYZ_dbl.X / modProgram.TerrainGridSpacing - X;
							InTileZ = XYZ_dbl.Z / modProgram.TerrainGridSpacing - Y;
							...
							TriHeightOffset = System.Convert.ToDouble(Map.Terrain.Vertices[X, Y + 1].Height * Map.HeightMultiplier);
							TriGradientX = System.Convert.ToDouble(Map.Terrain.Vertices[X + 1, Y + 1].Height * Map.HeightMultiplier - TriHeightOffset);
							TriGradientZ = System.Convert.ToDouble(Map.Terrain.Vertices[X, Y].Height * Map.HeightMultiplier - TriHeightOffset);
							XYZ_dbl.Y = (TriHeightOffset + TriGradientZ + (TriGradientX * (TerrainViewPos.X - TilePos.X) + TriGradientZ * (TilePos.Y - TerrainViewPos.Z) + (TriGradientX * TerrainViewVector.X - TriGradientZ * TerrainViewVector.Z) * TerrainViewPos.Y / TerrainViewVector.Y) / modProgram.TerrainGridSpacing) / (1.0D + (TriGradientX * TerrainViewVector.X - TriGradientZ * TerrainViewVector.Z) / (TerrainViewVector.Y * modProgram.TerrainGridSpacing));
							XYZ_dbl.X = TerrainViewPos.X + TerrainViewVector.X * (TerrainViewPos.Y - XYZ_dbl.Y) / TerrainViewVector.Y;
							XYZ_dbl.Z = TerrainViewPos.Z + TerrainViewVector.Z * (TerrainViewPos.Y - XYZ_dbl.Y) / TerrainViewVector.Y;
							InTileX = XYZ_dbl.X / modProgram.TerrainGridSpacing - X;
							InTileZ = XYZ_dbl.Z / modProgram.TerrainGridSpacing - Y;
		...
		public bool ScreenXY_Get_ViewPlanePos_ForwardDownOnly(int ScreenX, int ScreenY, double PlaneHeight, Matrix3D.Position.XY_dbl ResultPos)
		{
			double dblTemp = 0;
			Matrix3D.Position.XYZ_dbl XYZ_dbl = default(Matrix3D.Position.XYZ_dbl);
			Matrix3D.Position.XYZ_dbl XYZ_dbl2 = default(Matrix3D.Position.XYZ_dbl);
		...
		public void MouseOver_Pos_Calc()
		{
			...
			if (MouseLeftDownOverMinimap != null)
			{
				...
				{
					modMath.sXY_int Pos = new modMath.sXY_int((int)Conversion.Int(MouseOver.ScreenPos.X * Tiles_Per_Minimap_Pixel), (int) (Conversion.Int(MouseOver.ScreenPos.Y * Tiles_Per_Minimap_Pixel)));
			...
			else
			{
				...
				else
				{
					...
					if (ScreenXY_Get_ViewPlanePos(MouseOver.ScreenPos, MouseOverTerrain.Pos.Altitude, XY_dbl))
					{
						...
						MouseOverTerrain.Pos.Horizontal.Y = System.Convert.ToInt32(- XY_dbl.Y);
						if (Map.PosIsOnMap(MouseOverTerrain.Pos.Horizontal))
						{
				...
				if (Flag)
				{
					...
					MouseOverTerrain.Vertex.Alignment = new modMath.sXY_int(MouseOverTerrain.Tile.Normal.X + 1, MouseOverTerrain.Tile.Normal.Y + 1);
					...
					modMath.sXY_int SectorNum = Map.GetPosSectorNum(MouseOverTerrain.Pos.Horizontal);
		...
		public void Apply_Road()
		{
			...
			modMath.sXY_int Side_Num = MouseOverTerrain.Side_Num;
		...
		public void Apply_Road_Line_Selection()
		{
			...
			int A = 0;
			int B = 0;
			modMath.sXY_int Tile = MouseOverTerrian.Tile.Normal;
			modMath.sXY_int SideNum = new modMath.sXY_int();
			...
			{
				Map.Selected_Tile_A = new modMath.clsXY_int(Tile);
			}
		}
		...
		public void Apply_Terrain_Fill(modProgram.enumFillCliffAction CliffAction, bool Inside)
		{
			...
			modMath.sXY_int StartVertex = MouseOverTerrain.Vertex.Normal;
			...
			int A = 0;
			modMath.sXY_int[] SourceOfFill = new modMath.sXY_int[524289];
			int SourceOfFillCount = 0;
			int SourceOfFillNum = 0;
			int MoveCount = 0;
			int RemainingCount = 0;
			int MoveOffset = 0;
			modMath.sXY_int CurrentSource = new modMath.sXY_int();
			modMath.sXY_int NextSource = new modMath.sXY_int();
				...
				if (!(StopForCliff || StopForEdge))
				{
					if (Map.Terrain.Vertices[CurrentSource.X, CurrentSource.Y].Terrain == ReplaceType)
					{
							...
							& NextSource.Y >= 0 & NextSource.Y <= Map.Terrain.TileSize.Y)
						{
							...
							{
								StopForCliff = Map.SideHIsCliffOnBothSides(new modMath.sXY_int(CurrentSource.X, CurrentSource.Y));
							}
							else
							{
								StopForCliff = false;
							...
							& NextSource.Y >= 0 & NextSource.Y <= Map.Terrain.TileSize.Y)
						{
							if (CliffAction == modProgram.enumFillCliffAction.StopAfter)
							{
								StopForCliff = Map.SideHIsCliffOnBothSides(new modMath.sXY_int(CurrentSource.X - 1, CurrentSource.Y));
							}
							else
							{
								StopForCliff = false;
							...
							& NextSource.Y >= 0 & NextSource.Y <= Map.Terrain.TileSize.Y)
						{
							if (CliffAction == modProgram.enumFillCliffAction.StopAfter)
							{
								StopForCliff = Map.SideVIsCliffOnBothSides(new modMath.sXY_int(CurrentSource.X, CurrentSource.Y));
							}
							else
							{
								StopForCliff = false;
							...
							& NextSource.Y >= 0 & NextSource.Y <= Map.Terrain.TileSize.Y)
						{
							if (CliffAction == modProgram.enumFillCliffAction.StopAfter)
							{
								StopForCliff = Map.SideVIsCliffOnBothSides(new modMath.sXY_int(CurrentSource.X, CurrentSource.Y - 1));
							}
							else
							{
								StopForCliff = false;
		...
		public void Apply_Cliff()
		{
			...
			ApplyCliff.Angle = modMath.Clamp_dbl(Angle * modMath.RadOf1Deg, 0.0D, modMath.RadOf90Deg);
		...
		public void Apply_HeightSmoothing(double Ratio)
		{
			...
			ApplyHeightSmoothing.Offset.X = modMath.Clamp_int(PosNum.X - Radius, 0, Map.Terrain.TileSize.X);
			ApplyHeightSmoothing.Offset.Y = modMath.Clamp_int(PosNum.Y - Radius, 0, Map.Terrain.TileSize.Y);
			modMath.sXY_int PosEnd = new modMath.sXY_int();
			PosEnd.X = modMath.Clamp_int(PosNum.X + Radius, 0, Map.Terrain.TileSize.X);
			PosEnd.Y = modMath.Clamp_int(PosNum.Y + Radius, 0, Map.Terrain.TileSize.Y);
		...
		public void Apply_Gateway()
		{
			...
			if (modControls.KeyboardProfile.Active(modControls.Control_Gateway_Delete))
			{
				...
				{
					modMath.ReorderXY(Map.Gateways[A].PosA, Map.Gateways[A].PosB, Low, High);
					if (Low.X <= Tile.X 
						& High.X >= Tile.X 
						& Low.Y <= Tile.Y 
						& High.Y >= Tile.Y)
		...
		public void MouseDown(MouseEventArgs e)
		{
			...
			if (e.Button == System.Windows.Forms.MouseButtons.Left)
			{
				...
				if (IsViewPosOverMinimap(ScreenPos))
				{
					...
					modMath.sXY_int Pos = new modMath.sXY_int((int) (Conversion.Int(ScreenPos.X * Tiles_Per_Minimap_Pixel)), (int)Conversion.Int(ScreenPos.Y * Tiles_Per_Minimap_Pixel));
					Map.TileNumClampToMap(Pos);
					LookAtTile(Pos);
				}
				else
					...
					if (MouseOverTerrain != null)
					{
						...
						if (modTools.Tool == modTools.Tools.ObjectSelect)
						{
							...
							else
							{
								...
								Map.Unit_Selected_Area_VertexA = new modMath.clsXY_int(MouseOverTerrain.Vertex.Normal);
								MapView.DrawViewLater();
							}
						}
						...
						else if (modTools.Tool == modTools.Tools.TerrainSelect)
						{
							...
							{
								Map.Selected_Area_VertexA = new modMath.clsXY_int(MouseOverTerrain.Vertex.Normal);
								MapView.DrawViewLater();
							}
							else if (Map.Selected_Area_VertexB == null)
							{
								Map.Selected_Area_VertexB = new modMath.clsXY_int(MouseOverTerrain.Vertex.Normal);
								MapView.DrawViewLater();
							}
							else
							{
		...
		public void TimedActions(double Zoom, double Move, double Pan, double Roll, double OrbitRate)
		{
			...
			Matrix3D.Position.XYZ_dbl ViewAngleChange = default(Matrix3D.Position.XYZ_dbl);
			modMath.sXYZ_int ViewPosChangeXYZ = new modMath.sXYZ_int();
			...
			{
				FOV_Scale_2E_Change(System.Convert.ToDouble(- Zoom));
			}
			...
			{
				ViewPosChangeXYZ = new modMath.sXYZ_int();
				...
				if (modProgram.RTSOrbit)
				{
					if (modControls.KeyboardProfile.Active(modControls.Control_View_Forward))
					{
						AnglePY.Pitch = modMath.Clamp_dbl(AnglePY.Pitch + OrbitRate, System.Convert.ToDouble(- modMath.RadOf90Deg + 0.03125D * modMath.RadOf1Deg), modMath.RadOf90Deg - 0.03125D * modMath.RadOf1Deg);
						AngleChanged = true;
					}
					if (modControls.KeyboardProfile.Active(modControls.Control_View_Backward))
					{
					...
					if (modControls.KeyboardProfile.Active(modControls.Control_View_Left))
					{
						AnglePY.Yaw = modMath.AngleClamp(AnglePY.Yaw + OrbitRate);
						AngleChanged = true;
					}
					if (modControls.KeyboardProfile.Active(modControls.Control_View_Right))
					{
		...
		public void TimedTools()
		{
			...
			if (modTools.Tool == modTools.Tools.HeightSmoothBrush)
			{
				if (GetMouseOverTerrain() != null)
				{
					if (GetMouseLeftDownOverTerrain() != null)
					{
						...
						Apply_HeightSmoothing(modMath.Clamp_dbl(dblTemp * modMain.frmMainInstance.tmrTool.Interval / 1000.0D, 0.0D, 1.0D));
					}
				}
			}
			else if (modTools.Tool == modTools.Tools.HeightChangeBrush)
			{
				if (GetMouseOverTerrain() != null)
				{
					...
					if (GetMouseLeftDownOverTerrain() != null)
					{
						Apply_Height_Change(modMath.Clamp_dbl(dblTemp, -255.0D, 255.0D));
					}
					else if (GetMouseRightDownOverTerrain() != null)
					{
						Apply_Height_Change(modMath.Clamp_dbl(System.Convert.ToDouble(- dblTemp), -255.0D, 255.0D));
					}
				}
			}
		}
